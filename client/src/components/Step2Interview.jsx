...
const handleNext = async () => {
  ...
  // Speak before moving to the next question
...
setTimeLeft(questions[nextIndex].timeLimit);
  ...
  // Restart microphone after a short delay
  setTimeout(() => {
    if (isMicOn) {
      startMic();
    }
...
useEffect(() => {
  ...
  disabled={isSubmitting}
    whileTap={{ scale: 0.95 }}
    className="flex-1 bg-linear-to-r from-emerald-600 to-teal-500 text-white py-3 sm:py-4 rounded-2xl 
    shadow-lg hover:opacity-90 transition font-semibold disabled:bg-gray-500"
  >
...
>
  ...
  <button
    onClick={handleNext}
    className="w-full bg-linear-to-r from-emerald-600 to-teal-500 text-white py-3 rounded-xl shadow-md 
...
"Hello Sumit, welcome to your mock interview..."
        │
        ▼
...
Submit Answer
- Convert speech → text
- Send transcript to backend/OpenRouter
        │
        ▼
